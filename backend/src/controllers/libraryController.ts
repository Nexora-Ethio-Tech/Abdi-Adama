...
export const issueBook = async (req: Request, res: Response) => {
  ...
  try {
    await withRLS(req, async (client) => {
      ...
      await client.query(
        'INSERT INTO library_loans (book_id, student_id, due_date) VALUES ($1, $2, $3)',
...
export const returnBook = async (req: Request, res: Response) => {
  ...
  try {
    await withRLS(req, async (client) => {
      ...
      if (loan.returned_at) throw new Error('Book already returned');
      ...
      await client.query(
        'UPDATE library_loans SET returned_at = CURRENT_DATE, days_overdue = $1, fine_amount = $2 WHERE id = $3',
        [daysOverdue, fine, loan_id]
      );
