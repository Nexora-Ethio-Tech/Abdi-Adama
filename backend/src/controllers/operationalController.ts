...
export const createEvent = async (req: Request, res: Response) => {
  ...
  try {
    await withRLS(req, async (client) => {
      await client.query(
        `INSERT INTO events (title, event_date, type, branch_id, expiry_date)
         VALUES ($1, $2, $3, $4, $5)`,
        [title, event_date, type || 'Event', user.branch_id, expiry_date || event_date]
      );
    });
...
export const createInventoryItem = async (req: Request, res: Response) => {
  ...
  try {
    await withRLS(req, async (client) => {
      await client.query(
        `INSERT INTO inventory (name, category, quantity, condition, location, branch_id)
         VALUES ($1, $2, $3, $4, $5, $6)`,
        [name, category, quantity || 0, condition || 'Good', location, user.branch_id]
      );
    });
...
export const updateInventoryItem = async (req: Request, res: Response) => {
  ...
  try {
    await withRLS(req, async (client) => {
      await client.query(
        `UPDATE inventory SET name=$1, category=$2, quantity=$3, condition=$4, location=$5, updated_at=NOW()
         WHERE id=$6`,
        [name, category, quantity, condition, location, id]
      );
    });
  ...
  try {
    await withRLS(req, async (client) => {
      ...
      for (const entry of entries) {
        await client.query(
          `INSERT INTO schedules (teacher_id, section_id, subject, day, time_slot, branch_id)
           VALUES ($1, $2, $3, $4, $5, $6)`,
          [teacher_id, entry.section_id, entry.subject, entry.day, entry.time_slot, user.branch_id]
        );
      }
    });
...
// ─── GRADING CONFIG ──────────────────────────────────────────
export const getGradingConfig = async (req: Request, res: Response) => {
...
export const upsertGradingConfig = async (req: Request, res: Response) => {
  ...
  try {
    await withRLS(req, async (client) => {
      await client.query(
        ...
        [student_id, date, status, user.id]
      );
    });
...
export const getAttendanceBySection = async (req: Request, res: Response) => {
