    ...
    try {
        ...
        await withRLS(req, async (client) => {
            ...
            const configResult = await client.query(
                'SELECT admission_fee, is_open FROM registration_config WHERE branch_id = $1',
                [branch_id]
            );
    ...
    try {
        await withRLS(req, async (client) => {
            ...
            const appResult = await client.query(
                'SELECT * FROM pending_applications WHERE id = $1',
                [applicationId]
            );
 ...
 */
export const markExamPassed = async (req: Request, res: Response) => {
    const { applicationId } = req.params;
    const user = (req as any).user;
...
export const finalizeEnrollment = async (req: Request, res: Response) => {
    ...
    try {
        const studentId = await withRLS(req, async (client) => {
            // Get application details
            const appResult = await client.query(
                'SELECT * FROM pending_applications WHERE id = $1 AND status = $2',
 ...
 */
export const bulkSendExamNotification = async (req: Request, res: Response) => {
