...
export const createStudent = async (req: Request, res: Response) => {
  const {
    ...
    grade, section_id, dob, gender, parent_name, parent_phone,
    emergency_contacts,
  ...
  try {
    const result = await withRLS(req, async (client) => {
      ...
      if (section_id) {
        await client.query(
          'UPDATE academic_sections SET current_count = current_count + 1 WHERE id = $1',
          [section_id]
        );
      }
  ...
  try {
    const profile = await withRLS(req, async (client) => {
      ...
      const contacts = await client.query(
        'SELECT * FROM emergency_contacts WHERE student_id = $1', [result.rows[0].id]
      );
...
export const getStudentCommunicationLogs = async (req: Request, res: Response) => {
  const { id } = req.params;
  const user = (req as any).user;
  ...
  try {
    await withRLS(req, async (client) => {
      const oldResult = await client.query(
        'SELECT monthly_fee, bus_fee, penalty_fee, fee_status FROM students WHERE id = $1', [studentId]
      );
      const old = oldResult.rows[0];

      const fee_approval_status = fee_status === 'reduced' ? 'pending' : 'none';
      await client.query(
        `UPDATE students SET monthly_fee=$1, bus_fee=$2, penalty_fee=$3,
      ...
      await client.query(
        ...
        [studentId, `Fee update for student ${studentId}`, user.email,
...
// ─── SUBMIT GRADE ENTRY ───────────────────────────────────────
export const submitGradeEntry = async (req: Request, res: Response) => {
